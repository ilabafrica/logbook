...
class Checklist extends Model implements Revisionable {
	...
	*/
	public static function idByName($name=NULL)
	{
		if($name!=NULL){
			try 
			{
				$counter = Checklist::where('name', $name)->orderBy('name', 'asc')->count();
				if($counter > 0)
				{
	...
	*/
	public function countiesWithData()
	{
		$counties = [];
		$cIds = [];
		foreach (array_filter(array_unique($this->surveys()->lists('facility_sdp_id'))) as $value => $key)
		{
		    $scIds = [];
		    if(FacilitySdp::find($key)->facility_id)
