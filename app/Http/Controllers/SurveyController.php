...
class SurveyController extends Controller {
	...
	public function api($id)
	{
		...
		if($checklist->name == 'M & E Checklist')
			...
			$checklist_id = 207684;//205946;//94529;
    ...
    public function process($data, $checklist)
    {
        ...
        foreach ($data as $key => $value)
        {
        	...
        	*/
        	...
        	// dd("Started: ".Carbon::parse($value['start'])->toDateTimeString()." Ended: ".$end_date." Submitted: ".$submit_date);
        	//	Save survey at this point after checking for existence
        	//	Compare dates
        	// dd("I get here!");
        	if($max->lt(Carbon::parse($value['_submission_time'])))
        	{
        	    if(!Survey::where('checklist_id', $checklist_id)->where('facility_sdp_id', $facility_sdp_id)->where('longitude', $longitude)->where('latitude', $latitude)->where('qa_officer', $qa_officer)->where('comment', $comment)->where('date_started', $start_date)->where('date_submitted', $submit_date)->first())
	        	{
	        		$survey = new Survey;        	
	        		$survey->checklist_id = $checklist_id;
	        		$survey->facility_sdp_id = $facility_sdp_id;
