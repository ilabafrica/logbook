...
<div class="row">
    <div class="col-lg-12">
        <ol class="breadcrumb">
            ...
            <li class="active">{!! Lang::choice('messages.level', 1) !!}</li>
        </ol>
    </div>
</div>
...
<div class="panel panel-primary">
    ...
    <div class="panel-body">
        ...
        <div class="row">
            <div class="col-sm-12">
                <table class="table table-striped table-bordered table-hover {!! $levels->isEmpty()?'search-table':'' !!}">
                    <thead>
                        <tr>
                            <th>{!! Lang::choice('messages.name', 1) !!}</th>
                            <th>{!! Lang::choice('messages.description', 1) !!}</th>
                            <th>{!! Lang::choice('messages.range-lower', 1) !!}</th>
                            <th>{!! Lang::choice('messages.range-upper', 1) !!}</th>
                            <th></th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($levels as $level)
                        <tr @if(session()->has('active_level'))
                                {!! (session('active_level') == $level->id)?"class='warning'":"" !!}
                            @endif
                            >
                            <td>{!! $level->name !!}</td>
                            <td>{!! $level->description !!}</td>
                            <td>{!! $level->range_lower !!}</td>
