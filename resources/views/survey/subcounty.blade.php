    ...
    <div class="panel-body">
        ...
        <ul class="nav nav-tabs">
            ...
            @if(Auth::user()->hasRole('County Lab Coordinator') || (!(Auth::user()->hasRole('County Lab Coordinator')) && !(Auth::user()->hasRole('Sub-County Lab Coordinator'))))
                <li class="active"><a href="{!! url('survey/'.$checklist->id.'/subcounty') !!}">{!! Lang::choice('messages.sub-county-summary', 1) !!}</a></li>
            @endif
        ...
        <div class="tab-content">
            <br />
            {!! Form::open(array('url' => 'survey/'.$checklist->id.'/subcounty', 'class'=>'form-inline', 'role'=>'form', 'method'=>'POST')) !!}
            <!-- Tab panes -->
            <div class="row">
                <div class="col-sm-5">
            ...
            <p>
                ...
                <a href="{!! url('survey/'.$checklist->id.'/subcounty/download') !!}" class="btn btn-success" target=""><i class="fa fa-download"></i> {!! Lang::choice('messages.download-summary', 1) !!}</a>
            </p>
            <div class="row">
                <div class="col-sm-12">
                         ...
                         <tbody>                          
                            @foreach($subCounties as $subCounty)
                                <tr>
                                    ...
                                    <td>{!! $checklist->fsdps($checklist->id, NULL, $subCounty->id, NULL, NULL, $from, $toPlusOne)->count() !!}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
